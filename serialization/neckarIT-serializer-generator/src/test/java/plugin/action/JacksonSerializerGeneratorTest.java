...
public class JacksonSerializerGeneratorTest extends MultiFileTestCase {
  ...
  public void testWithPackage() throws Throwable {
    doTest( new PerformAction() {
      @Override
      public void performAction( VirtualFile rootDir, VirtualFile rootAfter ) throws Exception {
        PsiClass simple = myJavaFacade.findClass("it.neckar.open.test.WithPackage", GlobalSearchScope.allScope( getProject() ) );
        assertThat( simple ).isNotNull();
        assertThat( simple.getQualifiedName() ).isEqualTo( "it.neckar.open.test.WithPackage" );
