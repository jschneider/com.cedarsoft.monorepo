...
object Strings {
   ...
   */
  @JvmStatic
  fun cut(uncut: String, maxLength: Int): String {
    return if (uncut.length > maxLength) {
      uncut.substring(0, maxLength)
    } else {
      uncut
    }
  }
   ...
   *
   * @param uncut     the uncut string
   * @param maxLength the maxlength
   * @return the cut uncut or null if the given uncut has been null
   */
  @JvmStatic
  fun cutNull(uncut: String?, maxLength: Int): String? {
    return if (uncut == null) {
      null
    } else cut(uncut, maxLength)
  }
}
